    ...
    try {
      ...
      const sanitized = Array.isArray(data)
        ? data
            .filter((d) => d?.address)
            .map((d) => ({
              id: getAddress((d.address as string) || (d.id as string)) as Address,
              address: getAddress((d.address as string) || (d.id as string)) as Address,
              name: String(d.name || '').slice(0, 64) || short(d.address as string),
    ...
    <>
      ...
      <main className="wrap">
        ...
        <style jsx>{`
          ...
          .chip.danger { background: #fff1f2; border-col
